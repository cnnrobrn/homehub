/**
 * `social-materializer` worker entrypoint.
 *
 * Long-running HTTP server with `/health` + `/ready`. Materialization
 * runs daily via Railway cron (`src/cron.ts`). Keeping a long-lived
...
const SERVICE_NAME = 'worker-social-materializer';
